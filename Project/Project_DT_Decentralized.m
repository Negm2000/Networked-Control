...
if ~isempty(Kx)
    F_cl = F + G*Kx;
    disp("Check if the decentralized discrete system is stable (Stability LMI).")
    isStable(F_cl);
...
if ~isempty(Kx)
    F_cl = F + G*Kx;
    disp("Check if the decentralized discrete system is stable (Speed LMI).")
    isStable(F_cl);
    % VISUALIZATION B
...
if J.problem
    ...
    F_cl = F + G*Kx;
    disp("Check if the decentralized discrete system is stable (H2).")
    isStable(F_cl);
...
if sol.problem == 0
    ...
    disp("Check if the decentralized discrete system is stable (H-inf).")
    isStable(F_cl_hinf);
...
if ~isempty(Kx)
    ...
    disp("Check if the decentralized discrete system is stable (Damping LMI).")
    isStable(F_cl);
